...
When a bool flag like a weapon's "is shooting?" is set true in one place, removing whatever used to reset it back to false can silently create a permanently-stuck state instead of a fix. Removing BP_Pistol's self-resetting Delay correctly turned single-shot into continuous fire, but that Delay had also been the only place in the entire weapon that ever set "is shooting?" back to false, so after one hold-to-fire cycle it stayed true forever and blocked every later Start Shooting call. Before deleting logic that toggles a flag, grep the whole owning class for every Set node on that variable and confirm at least one of them still runs on the path that should end the state, not just the path that begins it.

If the editor's MCP connection drops with a connection-refused or connection-reset error and reconnects fine after simply waiting or relaunching, do not assume the previous crash was caused by whatever Blueprint edit was in flight. Check tasklist for more than one UnrealEditor.exe process on the same project first. A relaunch issued while a prior instance was still alive but temporarily unresponsive (mid-crash-recovery, mid-shader-compile, or just slow to exit) produces two editor processes fighting over the same project files, and that is enough on its own to cause the repeated crashes, corrupted-looking saves, and edits that mysteriously revert to an older state that this session hit. Kill every UnrealEditor.exe for the project and confirm exactly zero are running before relaunching a single clean instance.

Before wiring a new automatic call into an existing gated custom event, trace that event's actual condition logic first rather than assuming what it checks from its name. Reload Weapon sounds like it would check "is the clip empty", but its real gate is an AND of "is there any reserve ammo in inventory" and "is the clip not already full" - calling it automatically only from a branch that already implies both is what makes it safe, and that only became clear after tracing the AND node's two inputs back to their source variables.

After any save on a Blueprint that had a crash or connection drop nearby in time, do not trust that the save actually took effect just because the tool call returned success. Immediately do a fresh read of the graph and grep for the specific node IDs or connections just changed before doing anything else, including before running a PIE test. This session had a save silently fail to persist after a crash, and the very next PIE test then produced confusing symptoms that looked like a brand new Blueprint bug but were actually just the old, unwanted wiring still being live.

Repeated automated equip/fire/reload test cycles against the same live PIE session can quietly exhaust shared player state like the inventory's reserve ammo pool. When automated auto-reload testing seems to stop working after several earlier tests succeeded, check whether the underlying resource (reserve ammo, in this case) has simply run out before assuming the feature itself regressed - a fresh PIE session with an untouched inventory is the fastest way to tell the two apart. Also, a Blueprint function expecting a struct parameter (like S_Item_Structure) cannot be called from Python by passing the data asset itself - Python's NativizeStructInstance fails outright - so topping up test state that way needs the actual expected struct, not a shortcut.
...
A hitscan weapon's trace Start and its trace direction are two independent decisions and fixing one without the other creates a new, more confusing bug. Making the trace originate from the weapon's barrel location was correct and necessary, but pairing that Start point with the barrel's own raw rotation made the shot go wherever the gun model happened to be posed at that instant, which almost never matches the on-screen crosshair since the crosshair represents the camera's center, not the gun mesh's orientation. The fix that actually satisfies both a barrel-relative origin and crosshair-accurate hits is to keep the barrel as Start but compute the direction with FindLookAtRotation from that barrel location toward a point projected far out along the camera's own forward vector, so the shot visually leaves the gun yet converges on the same point the crosshair is showing. When validating a change like this, compare the function's returned rotation against the player controller's live GetControlRotation rather than trusting a screenshot alone, since a few tenths of a degree of expected convergence offset can look identical to a real bug in a still image.
