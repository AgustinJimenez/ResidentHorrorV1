...
Planned hand-pose tuning must remain first-person-only. Route `bIsFirstPerson` from the player into `/Game/ResidentHorrorV1/Demo/ABP_Character/ABP_Player`, blend an additive upper-body correction only on the first-person branch, and leave the existing third-person pose path unchanged. Prefer a layered blend beginning at the clavicle or upper-arm bones so locomotion and the lower body remain intact; apply weapon-specific grip offsets where necessary, then solve the support hand with IK after the pose correction. Cover hip fire, aim, crouch, movement, reload, and steep look-down angles in the validation matrix. This is the preferred next tuning step before introducing separate first-person geometry.
